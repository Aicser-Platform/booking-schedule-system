...
export function AvailabilityManager({ staffId }: AvailabilityManagerProps) {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const res = await fetch(`/api/staff/time-off/requests`);
      if (res.ok) {
        const data = (await res.json()) as TimeOffRequest[];
        setRequests(data);
        setRequestError(null);
      } else if (res.status === 403) {
        setRequestError("Access denied. Please sign in as staff.");
      } else {
  ...
  const handleSubmitRequest = async () => {
    if (!startUtc || !endUtc) {
      setRequestError("Please select a start and end time.");
      return;
    }
    ...
    try {
      const payload = {
        start_utc: new Date(startUtc).toISOString(),
        end_utc: new Date(endUtc).toISOString(),
        is_all_day: isAllDay,
  ...
  const requestsWithMetadata = useMemo(
    () =>
      requests.map((request) => {
        const payload = request.payload || {};
        const start = payload.start_utc
          ? new Date(payload.start_utc as string).toLocaleString()
          : null;
        const end = payload.end_utc
          ? new Date(payload.end_utc as string).toLocaleString()
          : null;
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Availability is managed by admins. Submit a time-off request for
            approval.
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setIsAllDay(value === "true")}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          {isLoadingRequests ? (
            ...
            <div className="rounded-lg border border-dashed p-6 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Submit a time-off request above to start the approval flow.
              </p>
            </div>
          ) : (
...
function LegacyAvailabilityManager({ staffId }: AvailabilityManagerProps) {
  void staffId;
  return null;
