...
export function BookingForm({ service, staff, customer }: BookingFormProps) {
  ...
  const effectivePrice = useMemo(() => {
    ...
    if (selectedStaff?.price_override !== null && selectedStaff?.price_override !== undefined) {
  ...
  const effectiveDeposit = useMemo(() => {
    ...
    if (selectedStaff?.deposit_override !== null && selectedStaff?.deposit_override !== undefined) {
  ...
  const fetchMonthAvailability = async (targetMonth: Date, staffId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/availability/slots-v2/month?service_id=${service.id}&year=${year}&month=${month}&timezone=${encodeURIComponent(
          timezone,
        )}&staff_id=${staffId}`,
        { credentials: "include" },
      )
      ...
      const data = (await response.json()) as Array<{ date: string; has_slots: boolean }>
  ...
  const fetchAvailableSlots = async (date: Date, staffId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/availability/slots-v2?service_id=${service.id}&date=${dateStr}&timezone=${encodeURIComponent(
          timezone,
        )}&staff_id=${staffId}`,
        { credentials: "include" },
      )
  ...
  const fetchNextAvailableDate = async (date: Date, staffId: string) => {
    try {
      const dateStr = format(date, "yyyy-MM-dd")
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/availability/slots-v2/next-available?service_id=${service.id}&timezone=${encodeURIComponent(
          timezone,
  ...
  const handleBooking = async () => {
    ...
    try {
      if (!customer.email) {
        setBookingError("Please add your email in your profile before booking.")
        return
      }
